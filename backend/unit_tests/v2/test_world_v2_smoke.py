...
from app.v2.world_v2 import generate_world_v2, step_world_v2
...
# unit_tests/v2/test_lithosphere.py/test_torque.py/test_healpix_grid.py for those.
_COARSE_KWARGS = dict(node_density=0.5, climate_density=0.5, fluid_density=0.5, num_plates=6)


@pytest.fixture(scope="module")
def stepped_world():
    world = generate_world_v2(seed=11, **_COARSE_KWARGS)
    for _ in range(3):
        step_world_v2(world, 3_000_000.0)
    return world


def test_generate_world_v2_produces_plates_with_finite_elevation():
    world = generate_world_v2(seed=11, **_COARSE_KWARGS)
...
def test_step_world_v2_advances_time_and_stays_finite(stepped_world):
    world = stepped_world
...
def test_step_world_v2_keeps_fluid_state_bounded(stepped_world):
    world = stepped_world
...
def test_step_world_v2_populates_hydrology_and_climate_caches(stepped_world):
    world = stepped_world
...
def test_isostasy_derived_elevation_matches_hc_hm_state_at_generation():
    ...
    plates_v2.py/lithosphere.py establish at construction time."""
...
def test_deform_applies_isostasy_as_a_delta_not_an_overwrite(stepped_world):
    ...
    notion of Hc/Hm -- see plates_v2.LithospherePlate.deform's own comment on why this must
    be a delta), `elevation` should generally have drifted *away* from a bare
