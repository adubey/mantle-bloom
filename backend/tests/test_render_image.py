...
def test_render_grid_arrays_cover_the_sphere_with_no_gaps():
    world = _world()
    xy, elevation, plate_id, lake_depth, half_w, half_h = render_image._render_grid_arrays(world, "behrmann", np.eye(3))
...
def test_render_grid_stays_gap_free_under_a_nontrivial_rotation():
    ...
    xy, elevation, plate_id, lake_depth, half_w, half_h = render_image._render_grid_arrays(world, "eckert4", rotation)
    assert len(xy) > 1000
    assert np.all(half_w > 0)
    assert np.all(half_h > 0)
